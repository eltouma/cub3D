...
/*   Updated: 2024/10/01 02:26:49 by caguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	*m = p;
	...
	while (1)
	{
		...
		if ((*game).map.tab[(int)(*m).y / TILE_SIZE][(int)((*m).x / TILE_SIZE)] == 'D' 
			&& is_side_door_h(rad, (*m).x) == 0)
			//&& (fabs((TILE_SIZE - rest((*m).x, TILE_SIZE))*tan(rad)) > TILE_SIZE/2) )// || 
			//fabs(-(rest((*m).x, TILE_SIZE))*tan(rad)) > TILE_SIZE/2 ))
		{
